...
struct RawValue {
  ...
  RawValue(Type type, Cli::OptBase *opt, string name, size_t pos = 0,
           const char *ptr = nullptr)
      : type(type), opt(opt), name(name), pos(pos), ptr(ptr) {}
};
...
struct Cli::OptIndex {
  ...
  void index(const Cli &cli, const string &cmd, bool requireVisible);
  void index(OptBase &opt);
  vector<OptKey> findNamedOpts(const Cli &cli, CommandConfig &cmd,
                               NameListType type, bool flatten) const;
  string nameList(const Cli &cli, const OptBase &opt, NameListType type) const;

  static string desc(const OptBase &opt, bool withMarkup = true);
  static const unordered_map<string, OptBase::ChoiceDesc> &
  choiceDescs(const OptBase &opt);

private:
  void indexName(OptBase &opt, const string &name, int pos);
  void indexShortName(OptBase &opt, char name, bool invert, bool optional,
                      int pos);
  void indexLongName(OptBase &opt, const string &name, bool invert,
                     bool optional, int pos);
};
...
struct Cli::Config {
  ...
  static GroupConfig &findGrpAlways(CommandConfig &cmd, const string &name);
...
static string displayName(const string &file) {
  ...
  char ext[_MAX_EXT];
  if (!_splitpath_s(file.c_str(), nullptr, 0, nullptr, 0, fname, sizeof fname,
                    ext, sizeof ext)) {
...
#else
#include <libgen.h>
static string displayName(string file) { return basename((char *)file.data()); }
#endif
...
template <typename T>
static void replace(vector<T> &out, size_t pos, size_t count, vector<T> &&src) {
...
//===========================================================================
// static
Cli::Config &Cli::Config::get(Cli &cli) { return *cli.m_cfg; }
...
//===========================================================================
// static
CommandConfig &Cli::Config::findCmdAlways(Cli &cli, const string &name) {
...
//===========================================================================
// static
GroupConfig &Cli::Config::findGrpAlways(CommandConfig &cmd,
                                        const string &name) {
...
Cli::Config::Config() {
  static size_t width =
      clamp<size_t>(Cli::consoleWidth(), kMinConsoleWidth, kMaxConsoleWidth);
  updateWidth(width);
}
...
void Cli::Config::updateWidth(size_t width) {
  ...
  this->minKeyWidth =
      kDefaultMinKeyWidth * (kDefaultConsoleWidth + width) / 2 / width;
...
//===========================================================================
bool Cli::OptBase::withUnits(long double &out, Cli &cli, const string &val,
  ...
  if (unit.empty()) {
    if (~flags & fUnitRequire)
      return true;
    cli.badUsage(*this, val, "Value requires suffix specifying the units.");
    return false;
  }
  ...
  if (i != units.end()) {
    ...
    cli.badUsage(*this, val, "Units symbol '" + unit + "' not recognized.");
    return false;
  }
}
...
//===========================================================================
void Cli::OptIndex::index(const Cli &cli, const string &cmd,
                          bool requireVisible) {
  *this = {};
...
//===========================================================================
vector<OptKey> Cli::OptIndex::findNamedOpts(const Cli &cli, CommandConfig &cmd,
                                            NameListType type,
                                            bool flatten) const {
  vector<OptKey> out;
  for (auto &&opt : cli.m_cfg->opts) {
  ...
  sort(out.begin(), out.end(),
       [](auto &a, auto &b) { return a.sort < b.sort; });
  return out;
}

//===========================================================================
static bool includeName(const OptName &name, NameListType type,
                        const Cli::OptBase &opt, bool flag, bool inverted) {
  if (name.opt != &opt)
    return false;
  if (flag) {
    if (type == kNameEnable)
...
//===========================================================================
string Cli::OptIndex::nameList(const Cli &cli, const Cli::OptBase &opt,
  ...
  sort(snames.begin(), snames.end(),
       [](auto &a, auto &b) { return a->second.pos < b->second.pos; });
  for (auto &&sn : snames) {
  ...
  // Value
  auto valDesc =
      opt.m_valueDesc.empty() ? opt.defaultValueDesc() : opt.m_valueDesc;
  if (optional) {
    list += foundLong ? "[=" : " [";
    list += valDesc + "]";
  } else {
...
void Cli::OptIndex::indexName(OptBase &opt, const string &name, int pos) {
  ...
  case '<':
    ...
    if (opt.m_command.empty() && (optional || opt.minSize() != opt.maxSize())) {
...
//===========================================================================
void Cli::OptIndex::indexShortName(OptBase &opt, char name, bool invert,
                                   bool optional, int pos) {
...
//===========================================================================
void Cli::OptIndex::indexLongName(OptBase &opt, const string &name, bool invert,
                                  bool optional, int pos) {
  bool allowNo = true;
...
void Cli::requireAction(Cli &cli, OptBase &opt, const string &) {
  ...
  const string &name =
      !opt.defaultFrom().empty() ? opt.defaultFrom() : "UNKNOWN";
...
//===========================================================================
static void helpOptAction(Cli &cli, Cli::Opt<bool> &opt,
                          const string & // val
) {
  if (*opt) {
...
static void defCmdAction(Cli &cli) {
  if (cli.commandMatched().empty()) {
    ...
    cli.fail(kExitSoftware, "Command '" + cli.commandMatched() +
...
Cli::Cli() : m_cfg(globalConfig()) { helpOpt(); }

//===========================================================================
Cli::Cli(const Cli &from)
    : m_cfg(from.m_cfg), m_group(from.m_group), m_command(from.m_command) {}

//===========================================================================
// protected
Cli::Cli(shared_ptr<Config> cfg) : m_cfg(cfg) { helpOpt(); }
...
//===========================================================================
Cli::Opt<bool> &Cli::helpOpt() { return *Config::findCmdAlways(*this).helpOpt; }
...
//===========================================================================
Cli::Opt<bool> &Cli::versionOpt(const string &version, const string &progName) {
...
//===========================================================================
Cli &&Cli::group(const string &name) && { return move(group(name)); }
...
//===========================================================================
Cli &&Cli::title(const string &val) && { return move(title(val)); }
...
//===========================================================================
Cli &&Cli::sortKey(const string &val) && { return move(sortKey(val)); }

//===========================================================================
const string &Cli::title() const { return Config::findGrpOrDie(*this).title; }
...
//===========================================================================
Cli &&Cli::action(function<ActionFn> fn) && { return move(action(fn)); }
...
//===========================================================================
Cli &&Cli::header(const string &val) && { return move(header(val)); }
...
//===========================================================================
Cli &&Cli::desc(const string &val) && { return move(desc(val)); }
...
//===========================================================================
Cli &&Cli::footer(const string &val) && { return move(footer(val)); }

//===========================================================================
const string &Cli::header() const { return Config::findCmdOrDie(*this).header; }

//===========================================================================
const string &Cli::desc() const { return Config::findCmdOrDie(*this).desc; }

//===========================================================================
const string &Cli::footer() const { return Config::findCmdOrDie(*this).footer; }
...
Cli &Cli::helpCmd() & {
  ...
  cli.opt<string>("[COMMAND]").desc("Command to show help information about.");
  cli.opt<bool>("u usage").desc("Only show condensed usage.");
  return *this;
}

//===========================================================================
Cli &&Cli::helpCmd() && { return move(helpCmd()); }
...
//===========================================================================
Cli &&Cli::unknownCmd(function<ActionFn> fn) && { return move(unknownCmd(fn)); }

//===========================================================================
Cli &Cli::helpNoArgs() & { return before(helpBeforeAction); }

//===========================================================================
Cli &&Cli::helpNoArgs() && { return move(helpNoArgs()); }
...
//===========================================================================
Cli &&Cli::cmdGroup(const string &name) && { return move(cmdGroup(name)); }
...
//===========================================================================
Cli &&Cli::cmdTitle(const string &val) && { return move(cmdTitle(val)); }
...
//===========================================================================
Cli &&Cli::cmdSortKey(const string &key) && { return move(cmdSortKey(key)); }
...
//===========================================================================
Cli &&Cli::before(function<BeforeFn> fn) && { return move(before(fn)); }
...
//===========================================================================
Cli &&Cli::envOpts(const string &var) && { return move(envOpts(var)); }
#endif
...
//===========================================================================
Cli &&Cli::responseFiles(bool enable) && { return move(responseFiles(enable)); }
...
//===========================================================================
istream &Cli::conin() { return *m_cfg->conin; }

//===========================================================================
ostream &Cli::conout() { return *m_cfg->conout; }
...
//===========================================================================
void Cli::addOpt(unique_ptr<OptBase> src) { m_cfg->opts.push_back(move(src)); }
...
//===========================================================================
bool Cli::parseExited() const { return m_cfg->parseExit; }
...
static bool loadFileUtf8(string &content, const fs::path &fn) {
  ...
  if (content[0] == '\xff' && content[1] == '\xfe') {
    ...
    auto tmp =
        (string)wcvt.to_bytes(base + 1, base + content.size() / sizeof *base);
    if (tmp.empty())
      return false;
    content = tmp;
  } else if (content.size() >= 3 && content[0] == '\xef' &&
...
//===========================================================================
static bool expandResponseFile(Cli &cli, vector<string> &args, size_t &pos,
                               vector<string> &ancestors) {
  string content;
  error_code ec;
  auto fn = args[pos].substr(1);
  auto cfn = ancestors.empty() ? (fs::path)fn
...
//===========================================================================
// static
vector<pair<string, double>> Cli::siUnitMapping(const string &symbol,
                                                int flags) {
...
//===========================================================================
Cli &&Cli::resetValues() && { return move(resetValues()); }
...
//===========================================================================
bool Cli::parseValue(OptBase &opt, const string &name, size_t pos,
                     const char ptr[]) {
...
//===========================================================================
void Cli::badUsage(const string &prefix, const string &value,
                   const string &detail) {
  string out;
...
//===========================================================================
void Cli::badUsage(const OptBase &opt, const string &value,
                   const string &detail) {
...
static bool parseBool(bool &out, const string &val) {
  static const unordered_map<string, bool> allowed = {
      {"1", true},      {"t", true},   {"y", true},    {"+", true},
      {"true", true},   {"yes", true}, {"on", true},   {"enable", true},

      {"0", false},     {"f", false},  {"n", false},   {"-", false},
      {"false", false}, {"no", false}, {"off", false}, {"disable", false},
  };
...
//===========================================================================
static int numMatches(OprCat cat, int avail, const OptName &optn) {
...
//===========================================================================
static bool assignOperands(RawValue *rawValues, size_t numRawValues, Cli &cli,
                           const Cli::OptIndex &ndx, int numPos) {
...
//===========================================================================
static bool badMinMatched(Cli &cli, const Cli::OptBase &opt,
                          const string &name = {}) {
  int min = opt.minSize();
  int max = opt.maxSize();
  string detail;
  ...
  cli.badUsage("Option '" + (name.empty() ? opt.from() : name) +
                   "' missing value.",
               {}, detail);
  return false;
}
...
bool Cli::parse(vector<string> &args) {
  ...
  assert(!args.empty() && "At least one argument (the program name) required.");
  ...
  enum {
    ...
    kFound,
    kUnknown
  } cmdMode = m_cfg->allowUnknown || m_cfg->cmds.size() > 1 ? kPending : kNone;
  ...
  for (; argPos < argc; ++argPos, ++arg) {
    ...
    if (*ptr == '-' && ptr[1] && moreOpts) {
      ...
      for (; *ptr && *ptr != '-'; ++ptr) {
        ...
        if (argName.opt->m_bool) {
          rawValues.emplace_back(RawValue::kOption, argName.opt, name, argPos,
                                 argName.invert ? "0" : "1");
          continue;
        }
        ptr += 1;
      ...
      if (argName.opt->m_bool) {
        auto val = true;
        if (equal && (argName.opt->m_flagValue || !parseBool(val, ptr))) {
    ...
    if (cmdMode == kPending && numPos == ndx.m_minOprs) {
      auto cmd = (string)ptr;
      bool noExtras = assignOperands(rawValues.data(), rawValues.size(), *this,
                                     ndx, numPos);
    ...
    rawValues.emplace_back(RawValue::kOperand, nullptr, string{}, argPos, ptr);
  ...
  OPTION_VALUE:
    if (*ptr || equal) {
      rawValues.emplace_back(RawValue::kOption, argName.opt, name, argPos, ptr);
      continue;
    }
    if (argName.optional) {
      rawValues.emplace_back(RawValue::kOption, argName.opt, name, argPos,
  ...
  if (cmdMode != kUnknown) {
    if (!assignOperands(rawValues.data() + precmdValues,
                        rawValues.size() - precmdValues, *this, ndx, numPos)) {
      return false;
    }
  }
...
//===========================================================================
bool Cli::parse(vector<string> &&args) { return parse(args); }
...
//===========================================================================
int Cli::exitCode() const { return m_cfg->exitCode; };

//===========================================================================
const string &Cli::errMsg() const { return m_cfg->errMsg; }

//===========================================================================
const string &Cli::errDetail() const { return m_cfg->errDetail; }

//===========================================================================
const string &Cli::progName() const { return m_cfg->progName; }

//===========================================================================
const string &Cli::commandMatched() const { return m_cfg->command; }

//===========================================================================
const vector<string> &Cli::unknownArgs() const { return m_cfg->unknownArgs; }

//===========================================================================
bool Cli::exec() {
  auto &name = commandMatched();
  auto cmdFn =
      commandExists(name) ? m_cfg->cmds[name].action : m_cfg->unknownCmd;
  ...
  if (cmdFn) {
    ...
    fail(kExitSoftware, "Command '" + name + "' found by parse not defined.");
    return false;
  }
}
...
//===========================================================================
bool Cli::exec(vector<string> &args) { return parse(args) && exec(); }
  ...
  for (;;) {
    ...
    for (;; ++ptr) {
      if (*ptr == ' ') {
        ...
        if (*eptr == '\a' && col.minWidth <= col.maxWidth &&
...
static size_t formatCol(string *outPtr, int *lines, const RawCol &col,
                        size_t startPos, size_t pos, size_t lineWidth) {
  auto &out = *outPtr;
  ...
  auto childIndent = startPos + wrapIndent(col.indent + col.childIndent, width);
  out.append(indent, ' ');
  pos += indent;
  for (auto ptr = col.text, eptr = ptr + col.textLen; ptr != eptr;) {
    ...
    if (pos + wordLen + 1 > lineWidth && pos > wordLen) {
      firstWord = true;
      *lines += 1;
      out += '\n';
...
static int formatLine(string *outPtr, const RawLine &raw, size_t lineWidth) {
  auto &out = *outPtr;
...
//===========================================================================
static vector<ChoiceKey>
getChoiceKeys(const unordered_map<string, Cli::OptBase::ChoiceDesc> &choices) {
...
//===========================================================================
static void writeUsage(string *outPtr, Cli &cli, const string &arg0,
                       const string &cmdName, bool expandedOptions) {
  auto &out = *outPtr;
  ...
  if (!ndx.m_shortNames.empty() || !ndx.m_longNames.empty()) {
    if (!expandedOptions) {
      ...
      auto namedOpts = ndx.findNamedOpts(cli, cmd, kNameNonDefault, true);
...
//===========================================================================
static void writeHelp(string *outPtr, Cli &cli, const string &progName,
...
//===========================================================================
int Cli::printHelp(ostream &os, const string &progName, const string &cmdName) {
  string raw;
...
unsigned Cli::consoleWidth(bool queryWidth) {
  ...
  if (queryWidth) {
    if (ioctl(STDOUT_FILENO, TIOCGWINSZ, &w) != -1) {
      ...
      if (w.ws_col)
        return w.ws_col; // LCOV_EXCL_LINE
    }
#if !defined(DIMCLI_LIB_NO_ENV)
