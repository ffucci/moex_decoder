...
public:
  ...
  template <typename T> Opt<T> &opt(Opt<T> &value, const std::string &names);
  ...
  template <typename T> OptVec<T> &optVec(const std::string &names);
  ...
  Opt<bool> &versionOpt(const std::string &ver,
  ...
  int printHelp(std::ostream &os, const std::string &progName = {},
                const std::string &cmd = {});
  int printUsage(std::ostream &os, const std::string &progName = {},
                 const std::string &cmd = {});
  // Same as printUsage(), except always lists all non-default options
  // individually instead of the [OPTIONS] catchall.
  int printUsageEx(std::ostream &os, const std::string &progName = {},
                   const std::string &cmd = {});

  void printOperands(std::ostream &os, const std::string &cmd = {});
  ...
  template <typename T> static std::string valueDesc();
  ...
  static std::vector<std::string> toArgv(size_t argc, const wchar_t *argv[]);
  ...
  template <typename... Args> static std::string toCmdlineL(Args &&...args);
  ...
  void badUsage(const std::string &prefix, const std::string &value = {},
                const std::string &detail = {});

  // Calls badUsage(prefix, value, detail) with prefix set to:
  //  "Invalid '" + opt.from() + "' value"
  void badUsage(const OptBase &opt, const std::string &value,
                const std::string &detail = {});

  // Calls badUsage with "Out of range" message and the low and high in the
  // detail.
  template <typename A, typename T>
  void badRange(A &opt, const std::string &val, const T &low, const T &high);
  ...
  // those actions.
  [[nodiscard]] bool parseValue(OptBase &out, const std::string &name,
  ...
  void prompt(OptBase &opt, const std::string &msg,
  ...
  void fail(int code, const std::string &msg, const std::string &detail = {});
...
private:
  static void defParseAction(Cli &cli, OptBase &opt, const std::string &val);
  static void requireAction(Cli &cli, OptBase &opt, const std::string &val);

  static std::vector<std::pair<std::string, double>>
  siUnitMapping(const std::string &symbol, int flags);
...
Cli::Opt<T> &Cli::opt(T *value, const std::string &names, const U &def) {
...
//===========================================================================
template <typename T>
Cli::OptVec<T> &Cli::optVec(std::vector<T> *values, const std::string &names) {
...
Cli::Opt<T> &Cli::opt(Opt<T> &alias, const std::string &names, const U &def) {
...
//===========================================================================
template <typename T> Cli::OptVec<T> &Cli::optVec(const std::string &names) {
  return optVec<T>(nullptr, names);
}

//===========================================================================
template <typename A, typename T>
void Cli::badRange(A &opt, const std::string &val, const T &low,
                   const T &high) {
...
//===========================================================================
template <typename A> A &Cli::addOpt(std::unique_ptr<A> ptr) {
  auto &opt = *ptr;
...
inline // static
    std::string
    Cli::valueDesc<DIMCLI_LIB_FILESYSTEM_PATH>() {
  return "FILE";
}
#endif
...
//===========================================================================
template <typename T>
[[nodiscard]] bool Cli::Convert::fromString(T &out,
...
//===========================================================================
template <typename T>
auto Cli::Convert::fromString_impl(T &out, const std::string &src, int, int,
                                   int) const -> decltype(out = src, bool()) {
  out = src;
  return true;
}
...
//===========================================================================
template <typename T, typename>
bool Cli::Convert::fromString_impl(T &out, const std::string &src, int, int,
                                   long) const {
  out = T{src};
  return true;
}
...
//===========================================================================
template <typename T>
auto Cli::Convert::fromString_impl(T &out, const std::string &src, int, long,
                                   long) const
    -> decltype(std::declval<std::istream &>() >> out, bool()) {
  m_interpreter.clear();
...
//===========================================================================
template <typename T>
bool Cli::Convert::fromString_impl(T &,                 // out
...
//===========================================================================
template <typename T>
[[nodiscard]] bool Cli::Convert::toString(std::string &out,
                                          const T &src) const {
...
//===========================================================================
template <typename T>
auto Cli::Convert::toString_impl(std::string &out, const T &src, int) const
    -> decltype(std::declval<std::ostream &>() << src, bool()) {
  m_interpreter.clear();
  m_interpreter.str({});
...
//===========================================================================
template <typename T>
bool Cli::Convert::toString_impl(std::string &out, const T &, long) const {
  out.clear();
  return false;
}
...
//===========================================================================
template <typename... Args> std::string Cli::toCmdlineL(Args &&...args) {
...
protected:
  ...
  bool withUnits(long double &out, Cli &cli, const std::string &val,
...
public:
  ...
  // they were added.
  A &choice(const T &val, const std::string &key, const std::string &desc = {},
  ...
  A &siUnits(const std::string &symbol = {},
  ...
  // Cli::fUnit* flags.
  A &anyUnits(std::initializer_list<std::pair<const char *, double>> units,
              int flags = 0 // Cli::fUnit * flags
  );
  ...
  A &prompt(const std::string &msg, // custom prompt message
...
protected:
  ...
  void act(Cli &cli, const std::string &value,
...
//===========================================================================
template <typename A, typename T>
inline void Cli::OptShim<A, T>::doParseAction(Cli &cli,
...
//===========================================================================
template <typename A, typename T>
inline void Cli::OptShim<A, T>::doCheckActions(Cli &cli,
                                               const std::string &val) {
...
template <typename A, typename T>
inline void
Cli::OptShim<A, T>::act(Cli &cli, const std::string &val,
                        const std::vector<std::function<ActionFn>> &actions) {
  auto self = static_cast<A *>(this);
  for (auto &&fn : actions) {
    fn(cli, *self, val);
...
//===========================================================================
template <typename A, typename T> A &Cli::OptShim<A, T>::show(bool visible) {
  m_visible = visible;
  return static_cast<A &>(*this);
}
...
//===========================================================================
template <typename A, typename T> A &Cli::OptShim<A, T>::finalOpt() {
...
//===========================================================================
template <typename A, typename T>
A &Cli::OptShim<A, T>::choice(const T &val, const std::string &key,
...
//===========================================================================
template <typename A, typename T> A &Cli::OptShim<A, T>::require() {
...
//===========================================================================
template <typename A, typename T> A &Cli::OptShim<A, T>::timeUnits(int flags) {
  return anyUnits(
      {
          {"y", 365 * 24 * 60 * 60},
          {"w", 7 * 24 * 60 * 60},
...
A &Cli::OptShim<A, T>::anyUnits(
    std::initializer_list<std::pair<const char *, double>> units, int flags) {
  return anyUnits(units.begin(), units.end(), flags);
}
...
template <typename A, typename T>
template <typename U>
auto Cli::OptShim<A, T>::checkLimits(Cli &cli, const std::string &val,
                                     const U &x, int)
    -> decltype((U)std::declval<T &>() > x) {
...
template <typename A, typename T>
template <typename U>
bool Cli::OptShim<A, T>::checkLimits(Cli &, const std::string &, const U &,
                                     long) {
  return true;
}
...
A &Cli::OptShim<A, T>::anyUnits(InputIt first, InputIt last, int flags) {
  if (m_valueDesc.empty()) {
    m_valueDesc =
        defaultValueDesc() + ((flags & fUnitRequire) ? "<units>" : "[<units>]");
  }
...
//===========================================================================
template <typename T>
Cli::Opt<T>::Opt(std::shared_ptr<Value<T>> value, const std::string &names)
    : OptShim<Opt, T>{names, std::is_same<T, bool>::value}, m_proxy{value} {}
...
//===========================================================================
template <typename T> inline void Cli::Opt<T>::assignImplicit() {
...
//===========================================================================
template <typename T>
Cli::OptVec<T>::OptVec(std::shared_ptr<ValueVec<T>> values,
                       const std::string &names)
    : OptShim<OptVec, T>{names, std::is_same<T, bool>::value}, m_proxy(values) {
  this->m_vector = true;
  this->m_minVec = 1;
  this->m_maxVec = -1;
}
...
//===========================================================================
template <typename T> inline Cli::OptVec<T> &Cli::OptVec<T>::size(int exact) {
  if (exact < 0) {
...
//===========================================================================
template <typename T> inline void Cli::OptVec<T>::assignImplicit() {
